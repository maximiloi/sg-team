...
import { APPOINTMENT_STATUS_COLORS } from '@/constants/statusLabels';
...
function CalendarAppointment({
  ...
  appointmentsByDate?: Map<string, string[]>;
}) {
  ...
  return (
    <DayPicker
      ...
      components={{
        ...
        DayButton: ({ ...props }) => (
          <CalendarAppointmentDayButton appointmentsByDate={appointmentsByDate} {...props} />
        ),
...
function CalendarAppointmentDayButton({
  className,
  day,
  modifiers,
  appointmentsByDate,
  ...
  return (
    <Button
      ...
      {...props}
    >
      <Link href={`/board/active/${dateKey}`} className="flex flex-col items-center gap-1">
        <span className="z-10">{day.date.getDate()}</span>
        {dayAppointments.length > 0 && (
          <div className="absolute bottom-1 flex flex-wrap justify-center space-x-1">
            {dayAppointments.map((status, index) => (
              <span
                key={index}
...
export { CalendarAppointment, CalendarAppointmentDayButton };
